...
import { User, Lock, Mail, Calendar, Save, Eye, EyeOff, Building, Briefcase } from 'lucide-react';
import recrutadoresService from '../../services/recrutadores';
import type { RecrutadorProfile } from '../../services/recrutadores';
import { useAuth } from '../../contexts/AuthContext';

export default function RecrutadorPerfil() {
  const { updateUser } = useAuth();
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [profile, setProfile] = useState<RecrutadorProfile | null>(null);
  ...
  async function carregarPerfil() {
    try {
      setLoading(true);
      const data = await recrutadoresService.getProfile();
      setProfile(data);
      setNome(data.nome);
    } catch (err) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const updatedProfile = await recrutadoresService.updateProfile(dados);
  ...
  function getPerfilLabel(perfil: string) {
    return perfil === 'admin' ? 'Administrador' : 'Recrutador';
  }
        ...
        <div className="card">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-gray-600">{getPerfilLabel(profile?.perfil || '')}</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="text"
                  value={getPerfilLabel(profile?.perfil || '')}
                  disabled
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-100 text-gray-500 cursor-not-allowed"
                />
              </div>
            ...
            {profile?.associado && (
              <div>
                ...
                <input
                  type="text"
                  value={profile.associado.razao_social}
                  disabled
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-100 text-gray-500 cursor-not-allowed"
                />
              </div>
