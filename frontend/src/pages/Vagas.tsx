...
const Vagas = () => {
  ...
  const carregarDados = async () => {
    try {
      setLoading(true);
      const [vagasData, areasData] = await Promise.all([
          ...
          {vagasFiltradas.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {vagasFiltradas.map((vaga, index) => (
                ...
                >
                  <Card className="card-shadow h-full border-0 bg-gradient-to-br from-card to-card/80 backdrop-blur-sm hover:scale-105 transition-all duration-300 group cursor-pointer">
                    <CardHeader>
